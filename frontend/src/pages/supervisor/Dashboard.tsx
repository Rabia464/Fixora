...
import { FileText, Clock, ClipboardCheck, Bell, ArrowRight } from 'lucide-react'
...
// ─── Stat Card (reused pattern from student dashboard) ────────────────────────
...
export function SupervisorDashboard() {
    ...
    async function load() {
      try {
        const [c, n] = await Promise.all([
          // Supervisor GET /complaints defaults to Open if no status specified
  ...
  const openCount = complaints.filter((c) => c.status === 'Open' || c.status === 'Reopened').length
  const underReviewCount = complaints.filter((c) => c.status === 'UnderReview').length
  const forwardedCount = complaints.filter((c) => c.status === 'Forwarded' || c.status === 'InProgress').length
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <Text variant="body-md" className="mt-1 text-neutral-500">
            Manage and review complaints from your hostel.
          </Text>
        </div>
        <Button
          variant="brand"
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <Text variant="body-sm" className="text-neutral-500">
              No complaints to review right now.
            </Text>
          </div>
        ) : (
          <Table>
            ...
            {notifications.map((n) => {
              ...
              return (
                <button
                  key={n.id}
                  type="button"
                  onClick={() => navigate(`/supervisor/complaints/${n.complaint_id}`)}
                  className={cn(
