...
## 10.1 Scalability
The Fixora database is built on PostgreSQL with UUID primary keys, normalized tables, and a well‑designed indexing strategy. These foundations allow the system to scale horizontally by adding read replicas or partitioning large tables, and vertically by provisioning larger instance sizes, without altering the core schema. As the number of users, complaints, and notifications grows, the existing foreign‑key relationships and partition‑ready design continue to support efficient queries and low latency.

## 10.2 Functional Expansion
The current schema is deliberately extensible, enabling future functional modules to attach additional data without breaking existing tables. Possible extensions include:

- **Image attachments** – storing file metadata (e.g., URI, MIME type) in a lightweight table linked to `complaints`.
- **Hostel inventory** – a table for room assets that can reference the `users` responsible for each item.
- **Laundry management** – a service‑request entity that can reuse the `complaints` workflow model.
- **Visitor management** – a visitor log table that relates to `users` as hosts.
- **Parcel management** – delivery records associated with a `user` and optionally a `complaint` for issue tracking.
- **Multi‑campus support** – a `campus` reference table that can be added to `users` and `complaints` to distinguish locations.

All of these scenarios can be accommodated by adding optional foreign‑key columns or separate tables that reference the existing primary keys, preserving backward compatibility.

## 10.3 AI Evolution
The present rule‑based AI recommendation engine stores its suggestions directly on the `complaints` record. Should a machine‑learning model be introduced, the architecture can retain the same columns (`ai_category`, `ai_priority`, `ai_department`) and simply replace the generation logic. Minimal schema extensions—such as an optional `ai_model_version` column—would suffice to track model upgrades while keeping historical data intact.

## 10.4 Integration Opportunities
The database design anticipates integration with external systems:

- **University authentication systems** – the `users.email` field aligns with institutional directories, allowing single‑sign‑on federation.
- **Email services** – notification records can be consumed by an email‑sending service that references `notifications.id`.
- **SMS gateways** – similar to email, the `notifications` payload can include a phone number for SMS delivery.
- **Mobile applications** – the same RESTful API that serves the web UI can be reused by mobile clients, relying on the unchanged schema.
- **Analytics dashboards** – the immutable `audit_logs` table provides a reliable source for BI tools without schema modification.

These integration points leverage the existing tables and do not require structural changes.

## 10.5 Summary
The current database design offers a robust, standards‑compliant foundation that supports the Version 1 workflow while remaining adaptable. Its normalized structure, comprehensive indexing, and careful separation of concerns make it straightforward to extend for higher load, additional services, or richer AI capabilities, ensuring the system can evolve alongside the university’s growing needs.
Edited database_design.md

**Database Design Summary**  

PostgreSQL was chosen for Fixora because it delivers strong ACID guarantees, rich relational features, and a mature ecosystem that aligns with the project’s need for reliable transactional handling, referential integrity, and scalable performance. The database schema mirrors the complete complaint lifecycle—from student submission through AI‑generated recommendation, supervisor review, maintenance updates, and final student confirmation—by linking users, complaints, notifications, and immutable audit logs through well‑defined foreign‑key relationships.

Normalization to third normal form eliminates redundancy, while foreign‑key constraints preserve the logical connections between roles, users, complaints, and related entities. Primary‑key uniqueness, NOT NULL mandates, and scoped UNIQUE constraints together enforce data consistency, and a thoughtful indexing strategy—covering primary keys, unique columns, foreign keys, and frequently queried fields—ensures fast lookups without sacrificing write efficiency.

Audit logs capture every state transition, and the complaint table stores AI recommendation fields, enabling traceability of both automated suggestions and supervisor overrides. Notifications are modeled as first‑class records, allowing asynchronous user alerts that remain tightly coupled to relevant complaints. Together these components provide a cohesive data backbone that supports the defined workflow while remaining auditable and observable.

Migrations managed by Alembic, strict security controls (least‑privilege database accounts, TLS, and parameterized queries), and the performance optimizations described in earlier sections guarantee that the system can evolve safely, remain secure, and deliver responsive interactions. The design integrates cleanly with the Clean Architecture outlined in *software_architecture.md*, keeping data access isolated behind repository interfaces and ensuring that domain logic remains independent of persistence concerns.

Finally, the schema’s modularity and extensible foreign‑key design position it to accommodate future scalability—such as higher user volumes, read replicas, or partitioning—and to support additional features (e.g., image attachments, inventory management, or expanded AI models) without requiring a fundamental redesign. In sum, the database design delivers a secure, maintainable, scalable, and extensible foundation for Version 1 of Fixora while preserving the flexibility needed for future growth.